...
(function (global) {
  ...
  // ход), а окрестностью мест; окно учителя (не меньше двадцати слов в сторону и
  ...
  function stripHiddenPickPrefix(s) {
    ...
    // реплея ЦЕЛИКОМ.
    // После Source может стоять строка-указание 'Note: …' (2026-09-22): сервер
    // ставит её, когда кусок контекста короткий, а полный текст у учителя есть
    // (supabase/functions/_shared/word-pick.ts LEX_CONTEXT_NOTE). Та же цена
    // промаха: не узнать её — ход пропал бы из ленты целиком.
    const tail = src.slice(openAt + 1);
    const m = /"\n(?:Source:[^\n]*\n)?(?:Note:[^\n]*\n)?\n/.exec(tail);
    return m ? tail.slice(m.index + m[0].length) : '';  // ничего после — ход и есть инструкция
  }

  // ── Пузырь хода заготовки ────────────────────────────────────────────────
  //
  // Вопрос, заданный заготовкой, показывается так, как его видит учитель на
  // следующих ходах: короткая строка заготовки, пустая строка, фраза человека
  // (решение владельца 2026-09-22). Сервер присылает эту замену готовой —
  // кадром хода (laterText) и полем беседы (later_text). Строка сервером
  // сведена в одну, поэтому граница — ПЕРВАЯ пустая строка.
  //
  // У ходов, записанных до 2026-09-22, вторая половина замены — собранный
  // вопрос со скрытым блоком выбранного слова; переписывать их нельзя, поэтому
  // блок снимается здесь, при показе.
  //
  // Возвращает { line, phrase } или null, если замены нет.
  function splitPresetLater(later) {
    const src = String(later == null ? '' : later);
    if (!src) return null;
    const at = src.indexOf('\n\n');
    if (at < 0) return { line: src.trim(), phrase: '' };
    return { line: src.slice(0, at).trim(), phrase: stripHiddenPickPrefix(src.slice(at + 2)) };
  }
  // Текст пузыря хода заготовки: строка и фраза через пустую строку.
  function presetBubbleText(parts) {
    if (!parts) return '';
