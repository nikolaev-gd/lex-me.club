...
(function (global) {
  ...
  // ── Опубликованный набор: не только при открытии ─────────────────────────
  //
  // Владелец правит настройки в расширении и отправляет их кнопкой публикации
  // у полосы. `enterApp` забирает отправленное ОДИН раз, и другого повода
  // сходить за ним у страницы не было вовсе: открытая раньше публикации,
  // она продолжала работать прежними значениями до перезагрузки. У программы
  // для Мака то же самое и заметнее: её окно живёт сутками, и «перезагрузить
  // страницу» там уже не жест человека, а перезапуск программы. Так и вышло
  // 2026-09-10 с распознавалкой диктовки: в расширении живая, на странице и
  // Маке — прежняя, и никакой видимой причины.
  //
  // Повод — возвращение окна на передний план. Событий ДВА, и оба нужны:
  // вкладка браузера уходит в скрытые и возвращается через visibilitychange,
  // а окно программы для Мака остаётся видимым и меняет только фокус.
  //
  // Не чаще, чем раз в READOPT_MIN_MS: щёлкать между окнами — обычное дело, и
  // каждый щелчок не должен стоить запроса. Первый возврат после перерыва
  // проходит сразу — именно он и нужен, когда владелец отправил набор и
  // перешёл сюда посмотреть.
  //
  // Применение НИЧЕГО не перерисовывает, и это не упущение: всё, что приезжает
  // набором (модель, слоты промптов, ручки, распознавалка диктовки), читается
  // в тот миг, когда понадобилось, — на отправке хода и на нажатии микрофона.
  // Поэтому тихая запись в хранилище и есть весь эффект.
  const READOPT_MIN_MS = 30 * 1000;
  let readoptAt = 0;
  let readoptBusy = false;
  let readoptWatching = false;

  async function readoptPublished() {
    if (readoptBusy) return;
    if (Date.now() - readoptAt < READOPT_MIN_MS) return;
    readoptAt = Date.now();
    readoptBusy = true;
    try {
  ...
  function watchPublished() {
    if (readoptWatching) return;
    readoptWatching = true;
    // Отсчёт с этого мига: `enterApp` только что сходил за набором сам, и
    // возврат фокуса в ближайшие полминуты повторять запрос незачем.
    readoptAt = Date.now();
    document.addEventListener('visibilitychange', () => {
      if (document.visibilityState === 'visible') readoptPublished();
    });
    global.addEventListener('focus', () => readoptPublished());
  }
  ...
  async function enterApp() {
    ...
    // Дальше набор перечитывается на возврате окна — см. watchPublished выше.
    // Ставится ПОСЛЕ первого захода, чтобы отсчёт паузы шёл от него.
    watchPublished();
