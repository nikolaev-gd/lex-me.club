...
//
// Что в строке, слева направо (порядок — решение владельца 2026-09-25):
//   1. кнопка копирования — под КАЖДЫМ текстовым ответом, копирует текст
//      ответа в том виде, в каком его написала модель (разметка как есть);
//   2. переключатель версий «‹ 1 / 2 ›» — если версий больше одной;
//   3. кнопка выбора модели — подпись «модель · уровень» ответа; нажатие
//      открывает меню моделей окна, выбор переспрашивает этот ответ выбранной
//      моделью (сам переспрос и меню — у окна, модуль их не знает).
// Под своим сообщением впереди всех ещё карандаш правки.
//
// Цены в строке нет: цена под ответом — прибор разработчика, её ставит само
// окно правее строки (расширение, shared.js pricePill).
//
...
(function (global) {
  ...
  function setModel(row, spec) {
    ...
    if (!btn) {
      ...
      // Модель — последней в строке, за копированием и переключателем версий.
      row.appendChild(btn);
    }
    if (spec.title) {
  ...
  /**
   * Переключатель версий: «‹ 1 / 2 ›», стоит за копированием и перед
   * кнопкой модели. Версия одна — переключателя нет вовсе.
   *
  ...
  function setVersions(row, spec) {
    ...
    if (!box) {
      ...
      // Перед кнопкой модели, если она уже стоит: модель всегда последняя.
      row.insertBefore(box, row.querySelector(':scope > .lex-answer-row-model'));
    }
