...
// требуется.
//
// В приложении `window.open` перехватывается оболочкой и уходит в Safari — и
// для кассы провайдера это ПРАВИЛЬНО: там человек вводит карту, и он должен
// видеть адресную строку и настоящий сертификат, а не безымянное окно внутри
// чужого приложения.
//
// ── Чего здесь нет ──────────────────────────────────────────────────────────
  ...
  const WcTopup = {
    open(ctx) {
      ...
      pay.addEventListener('click', async () => {
        ...
        // жесту человека — и любой браузер такое окно заблокирует. Внутри
        // приложения это менее заметно, но в обычном браузере ломается совсем.
