...
//
// Хранилища с аккаунтными данными три, но эта функция стирает только два:
//   • lex_webchat (kv)          — перечислить и снести всё, кроме device-ключей;
//   • lex_webchat_images        — картинки сообщений, на сервере их нет вовсе.
// Третье, localStorage, run() ниже не трогает вовсе — и это не пропуск, а
// расчёт: у lex-me.club общий origin с /account и /checkout, и localStorage.clear()
// снёс бы заодно их состояние. Пропуск (lexSession) снимает сам выход, своим
// кодом, до вызова этой функции; wcDebug принадлежит устройству и переживает
// смену человека сознательно.
(function (global) {
  'use strict';

  // Ключи kv, которые переживают смену человека. Всё остальное — его.
  const DEVICE_SCOPED = new Set([
    // Идентификатор УСТАНОВКИ. Им подписана строка сеанса (sessions.user_id —
    // колонка про устройство, а не про человека, см. createSession в
    // wc-history.js): «этот сеанс завели здесь», а не на телефоне. Реплик он
    // больше не подписывает — их пишет сервер. К тому, кто вошёл, он
    // отношения не имеет и при смене человека не меняется.
    'wcDeviceId',
