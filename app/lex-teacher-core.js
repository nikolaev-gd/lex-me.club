    ...
    async function streamExplainWord(word, context, videoTitle, videoUrl, tabId, requestId, providerId, modelId, inboundClickId, chatOptions, videoId, knobs, wordPromptOverride) {
      ...
      const proxy = useProxy ? {
        ...
        meta: {
          ...
          // за материал у этой беседы — расшифровка ролика, текст страницы или
          // ничего. Рядом лежащий videoId для этого не годится дважды: он
          // обрезан extractRealVideoId до голого id ролика, а в вебе и на
          // айфоне вообще несёт ключ беседы.
          //
          ...
          // беседы про страницу — там материал выводится из ключа.
          materialVideoId: (chatOptions && chatOptions.materialVideoId) || null,
          // Человек открепил страницу крестиком до первой реплики. Сервер о
          // таком решении иначе не узнает: беседа остаётся привязанной к
          // странице, а материал в запрос идти не должен.
          material: (chatOptions && chatOptions.materialOff) ? 'off' : null,
          ...
          // ────────────────────────────────────────────────────────────────
          // Номер операции и всё, что из него следует. Ставится ПОСЛЕ условных
          // спредов выше: promptSlot уже однажды тихо затирали таким спредом,
          // и повторять это не хочется.
          //
          // opId — метка ОДНОГО НАЖАТИЯ человека. Чеканится поверхностью в
          // момент нажатия, а не здесь: тут мы уже за всеми гейтами и в
          // service worker'е, а номер обязан пережить повтор отправки. Из него
          // сервер считает уиды реплик — <opId>:u у человека, <opId>:a у
          // учителя — по тому же правилу, что и поверхность. Совпадут они
          // потому, что считаются из одного числа, а не потому, что кто-то
          // кого-то спросил.
          //
          // ⚠ opId — это НЕ turnRef выше. turnRef чеканится на каждый вызов
          // (переспрос даёт новый), сервер валидирует его голой uuid-регуляркой
          // и кладёт в журнал вызовов ради досылки клиентского времени.
          // Переиспользовать одно под другое нельзя ни в ту, ни в другую
          // сторону.
          opId: (chatOptions && chatOptions.opId) || null,
          ...
          act: (chatOptions && chatOptions.act) || 'send',
          // Момент нажатия по часам устройства. Сервер мог бы поставить своё
