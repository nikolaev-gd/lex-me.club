...
(function (root) {
  ...
  // (docs/spec/80-devtools.md, «Как пишутся деньги»): ноль — «$0»; от цента —
  // два знака; меньше цента — столько знаков, чтобы была видна первая
  // значащая цифра; округление, перешедшее через цент, пишется центами;
  // сервер цену не прислал — «$?».
  function format(usd) {
    if (usd == null) return '$?';
