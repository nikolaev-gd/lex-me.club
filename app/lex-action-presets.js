...
// отправки (problemOf ниже, признак hasLine). Сама строка приезжает в списке
// пилюль (shortLine, 2026-09-23) — по ней пузырь хода рисуется сразу при
// нажатии (laterOf ниже); промпт заготовки на устройство не приезжает. Прежний режим отдельной ветки снят; его код — тег
...
(function (global) {
  ...
  function presetItem(p) {
    return {
      ...
      // Текст короткой строки — только для показа пузыря до первого кадра
  ...
  // Пузырь хода заготовки СРАЗУ при нажатии: строка, пустая строка, фраза — та
  // же форма, что собирает сервер (presetLaterText в llm-proxy), с тем же
  // сведением строки в одну. Только показ: первый кадр ответа (laterText)
  // перерисует пузырь серверной версией, и что лечь в беседу, решает он.
  // '' — строки на устройстве нет, пузырь ждёт кадр.
  function laterOf(scope, id, phrase) {
    const hit = current(scope).find((p) => p.id === id);
    const line = String((hit && hit.shortLine) || '').replace(/\s+/g, ' ').trim();
    return line ? `${line}\n\n${phrase == null ? '' : phrase}` : '';
  }
  ...
  // ЗАГОТОВКА ПО УМОЛЧАНИЮ ДЛЯ ВЫБРАННОГО МАТЕРИАЛА (решение владельца
  // 2026-09-23). Выбраны слова (нажатием или карандашом) и человек жмёт
  // обычную отправку — ход уходит этой заготовкой, ровно как нажатие её
  // пилюли. Узнаём её по слоту, а не по имени: имя владелец может поменять.
  // Сегодня это «translate» — статический слот chatB2 ячейки nativePrompts.
  //
  // Возвращает строку списка, только если заготовка есть и отправима
  // (problemOf === null); иначе null — и тогда отправка идёт обычным
  // вопросом, а не упирается в отказ.
  //
  // TODO: правило общее для всех поверхностей (айфон держит свою копию слота
  // в ChatScreen.swift) — место ему на сервере, отметкой в списке `presets`.
  const PICK_DEFAULT_SLOT = 'chatB2';
  function pickDefaultOf(scope) {
    if (problemOf(scope, PICK_DEFAULT_SLOT) !== null) return null;
    return current(scope).find((p) => p.id === PICK_DEFAULT_SLOT) || null;
  }
